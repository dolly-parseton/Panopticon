//! End-to-end pipeline: variables, steps, returns, hooks, and validation.
//!
//! Shows the typestate flow — build with `Pipeline::default`, register
//! variables and steps, then `compile()` to check every reference before
//! `run().wait()` executes. The happy path attaches the standard hooks
//! (Logger, Profiler, EventLog, Timeout, StoreValidator) and reads the
//! profiler/event-log handles after completion. The error cases show what
//! `compile()` rejects: unresolved refs, forward refs, and duplicate names.
