//! Iterating a map with `iter_map`.
//!
//! The body closure is handed the current `key` and `value` binding names —
//! feed them to `Param::reference` to read each entry. Parent variables
//! remain accessible inside the body, and steps after the loop see the
//! final iteration's outputs in the parent store.
