//! Iterating an array with `iter_array`.
//!
//! The body closure receives the current `index` and `item` binding names —
//! pass them to `Param::reference` to read the current element. Parent-scope
//! variables are still in scope inside the body, so you can combine iteration
//! state with outer values. Source and body references are resolved at
//! `compile()` time.
