...
///
/// Extensions are shared, read-only services registered in the draft
/// phase via
/// [`Pipeline::extension`](crate::prelude::Pipeline#method.extension)
/// and looked up at runtime by custom operations through
/// [`Context::extension`](crate::extend::Context). The `Clone + Send +
/// Sync + 'static` bounds let the runtime hand each step its own
/// handle without coordinating concurrent access.
///
/// For stateful extensions that need interior mutability, wrap them in
/// `Arc` and use internal synchronisation (`Mutex`, `RwLock`, etc.):
///
/// ```ignore
/// struct TokenCache {
...
impl Extensions {
    ...
    pub(crate) fn contains_by_id(&self, name: &str, type_id: TypeId) -> bool {
        self.entries.contains_key(&ExtensionKey {
            name: name.to_string(),
            type_id,
        })
    }
...
mod tests {
    ...
    #[test]
    fn contains_by_id_matches_name_and_type() {
        let mut ext = Extensions::new();
        ext.insert("app_config", AppConfig { debug: false });

        assert!(ext.contains_by_id("app_config", TypeId::of::<AppConfig>()));
        // Right name, wrong type
        assert!(!ext.contains_by_id("app_config", TypeId::of::<Arc<TokenStore>>()));
        // Right type, wrong name
        assert!(!ext.contains_by_id("other", TypeId::of::<AppConfig>()));
    }
