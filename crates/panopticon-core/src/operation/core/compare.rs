...
/// Built-in operation that compares two values and emits a boolean.
///
/// **Inputs:**
/// - `left` (Any, required) — left-hand side of the comparison.
/// - `op` (Text, required) — one of `eq`, `neq`, `gt`, `gte`, `lt`,
///   `lte` (case-insensitive).
/// - `right` (Any, required) — right-hand side of the comparison.
/// - `name` (Text, optional) — the output variable name (default
///   `result`).
///
/// **Outputs:**
/// - An operation-scoped boolean output with the comparison result.
///
/// `Null`, `Boolean`, and `Text` only support `eq` and `neq`;
/// `Integer` and `Float` support all operators and cross-promote
/// integers to floats when comparing. Incompatible type combinations
