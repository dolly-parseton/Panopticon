...
/// Built-in operation that extracts a single element from an array or map.
///
/// **Inputs:**
/// - `source` (Any, required) — an array or map store entry to read from.
/// - `key` (Any, required) — an integer index for arrays, a text key for
///   maps; the shape must match `source`.
/// - `name` (Text, optional) — the output variable name (default
///   `result`).
///
/// **Outputs:**
/// - An operation-scoped output holding the extracted entry.
