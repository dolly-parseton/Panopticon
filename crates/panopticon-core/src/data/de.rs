...
/// The error type produced by `serde` deserialisation of [`StoreEntry`]
/// values, including `Pipeline<Complete>::deserialize_returns`.
///
/// Wraps a single human-readable message — the crate does not expose a
/// structured error taxonomy for `serde` failures, since the
/// deserialiser delegates to the target type's own `Deserialize` impl
/// for most of the work. Available only under the `serde` feature.
#[derive(Debug)]
